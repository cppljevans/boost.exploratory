...
namespace boost { namespace spirit { namespace x3
{
    ...
    struct real_parser : parser<real_parser<T, RealPolicies> >
    {
        ...
        {
            boost::trace_scope ts(stringify("x3/numeric/real::",__func__,"(yes T)"));
            x3::skip_over(first, last, context);
            bool const result=extract_real<T, RealPolicies>::parse(first, last, attr_, policies);
            std::cout<<":result="<<result<<":attr_="<<attr_<<";\n";
            return result;
        }
        ...
        {
            boost::trace_scope ts(stringify("x3/numeric/real::",__func__,"(not T)"));
            // this case is called when Attribute is not T
            T attr_;
            bool const result=parse(first, last, context, unused, attr_);
            std::cout<<":result="<<result;
            if (result)
            {
                traits::move_to(attr_, attr_param);
                std::cout<<":attr_param="<<attr_param<<";\n";
            }
            std::cout<<";\n";
            return result;
        }
