...
#pragma push_macro("BOOST_NOEXCEPT_IF")
#define BOOST_NOEXCEPT_IF(_)
#include <boost/spirit/home/x3/support/ast/forward_ast.hpp>
#pragma pop_macro("BOOST_NOEXCEPT_IF")
...
double CalcArithExpr(Iterator const &first, Iterator last) 
{
    ...
    auto astEval = hana::fix
      ( [](auto self, auto expr) -> double 
        {
        // hana::overload calls a lambda corresponding to the type in the variant
          ...
          , [&](ast::signed_ const &x) -> double 
              {
                  double rhs = std::visit(self, x.operand_);
                  switch (x.sign) {
                      case '-': return -rhs;
                      case '+': return +rhs;
                  }
