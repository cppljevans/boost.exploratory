...
namespace boost { namespace spirit { namespace x3
{
    ...
    struct rule : parser<rule<ID, Attribute, force_attribute_>>
    {
        ...
        bool parse(Iterator& first, Iterator const& last
          , Context const& context, unused_type, Attribute_& attr) const
        {
          #if defined(USE_TRACE_SCOPE)
            boost::trace_scope _trace_scope(std::string{"rule::"}+__func__+":"+name);
            #if 0
              std::cout<<"Attribute      rule  ="<<demangle_type<Attribute>()<<";\n";
              std::cout<<"Attribute_     actual="<<demangle_type<Attribute_>()<<";\n";
            #endif
