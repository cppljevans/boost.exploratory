...
namespace boost { namespace spirit { namespace x3 { namespace detail
{
    ...
    struct pass_sequence_attribute<Parser, Attribute
      ...
      , typename Attribute
      , typename Context
      , bool has_left=traits::has_attribute<L, Context>::value
      , bool has_right=traits::has_attribute<R, Context>::value
      >
      ...
      )
    {
        ...
        std::cout<<"demangle_fmt_type(parser.right)=\n"<<demangle_fmt_type(parser.right)<<";\n";
        std::cout<<"demangle_fmt_type(r_attr)=\n"<<demangle_fmt_type(r_attr)<<";\n";
        std::cout<<"demangle_fmt_type<RContext>()=\n"<<demangle_fmt_type<RContext>()<<";\n";

        Iterator save = first;
        bool result=true;
    #ifdef USE_ATTR_QVM_VEC
        using rcontext_skip=boost::qvm::vec<int32_t,2>;
        bool constexpr use_qvm_workaround=
      #ifdef USE_QVM_WORKAROUND
          true
      #else
          false
      #endif
          ;
        if constexpr(use_qvm_workaround && std::is_same<rcontext_skip,RContext>::value)
        {   std::cout<<__FILE__<<':'<<__LINE__<<":rcontext_skip yes;\n";
        }
        else
        {
      ...
      , traits::container_attribute)
    {
        ...
        Iterator save = first;
        bool result=parse_sequence_container(parser.left, first, last, context, rcontext, attr);
        std::cout<<"left result="<<result<<";\n";
        if (result)
        {   result=parse_sequence_container(parser.right, first, last, context, rcontext, attr);
            std::cout<<"right result="<<result<<";\n";
        }
        if(!result) first = save;
        return result;
