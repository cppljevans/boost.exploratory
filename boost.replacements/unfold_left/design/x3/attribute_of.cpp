...
int main()
{
  ...
  #endif
    boost::trace_scope ts(stringify(__func__,':',__LINE__));
  ...
  #if 1
    {
        boost::trace_scope ts(stringify(__func__,':',__LINE__,":operator>>"));
        using parser_type = 
          decltype
          (  attr(unused) 
          >> attr(type_i<1>()) 
  ...
  #if 0
    {
        boost::trace_scope ts(stringify(__func__,':',__LINE__,":parser::r1,parser::rhs_attr_t"));
        ...
        std::cout
          <<"\ndemangle_type(rhs_attr_v)="<<demangle_fmt_type(rhs_attr_v)
          <<";\n";
    }
  #endif
    return 0;
