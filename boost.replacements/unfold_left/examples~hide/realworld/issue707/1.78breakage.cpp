 ...
 */
//Changes:
//  added several macros to test different ways to process attributes:
//===================
#include <iostream>
#include <string>
#include <vector>
...
transform_attribute
  ...
  >
  {
      ...
      { auto from_attr=boost::fusion::tie
        ( unused  //'{'
        , to_attr //x3::int_ % ','
        , unused  //'}'
        )
      ; std::cout<<"type(result)="<<demangle_fmt_type(from_attr)<<";\n"
      ; return from_attr
      ;}
      
      template<typename From>
      static void post(to_attribute_type& to_attr, From& from_attr)
      {  //do nothing since the pre function tied to_attr to from_attr
      }
    #else  
