#ifndef BOOST_IOSTREAMS_FILTER_INDENT_FILTER_CRTP_HPP
#define BOOST_IOSTREAMS_FILTER_INDENT_FILTER_CRTP_HPP
...
class indent_filter_crtp
{
 public:
    ...
    bool put_(Sink& dest, char_type c)
    {
        if (c == '\n')
            linestart_ = true;
        else
            if (linestart_) {
                for(int n=0; n<indent_; ++n)
                    static_cast<Derived*>(this)->put_concrete(dest, ' ');
                linestart_ = false;
            }
